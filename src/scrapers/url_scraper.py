...
Scrapes player profile URLs from Futbin's player database.
...
# Manually setted up Futbin website to find all 83+ OVR non-goalkeepers tradeable rated players URLs
BASE_URL = (
...
def scrape_player_urls(base_url: str, total_pages: int) -> List[str]:
    ...
    for page in range(1, total_pages + 1):
        ...
        logger.info("Scraping page %s/%s", page, total_pages)
        ...
        # Acknowledge if there was an error on a specific page
        except requests.exceptions.RequestException as e:
            logger.error("Error on page %s: %s", page, e)
            failed_pages.append(page)
            continue

    # Acknowledge the number of pages that failed to scrape
    if failed_pages:
        logger.warning("%s pages failed to scrape.", failed_pages)
...
if __name__ == "__main__":
    ...
    logger.info("Scraped %s URLs to %s", len(urls), OUTPUT_FILE)
