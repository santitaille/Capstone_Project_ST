"""
OLS Coefficient Analysis for EA FC 26 Player Price Prediction.

Analyzes relationship between player attributes and prices using OLS regression.

Generates:
* Table: OLS coefficients with confidence intervals and p-values (CSV)
...
def main() -> None:
    ...
    try:
        ...
        top3 = ", ".join(
            [
                f"{row['feature']} ({row['coefficient']:+.2f})"
        ...
        # Summary
        logger.info("OLS Analysis Summary:")
        logger.info("-" * 80)
        logger.info("R²: %.3f", r2_price)
        logger.info(
            "%d/%d features significant (p < 0.05)",
            significant_count,
            len(feature_names),
        )
        logger.info("")
