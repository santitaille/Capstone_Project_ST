"""
Feature Engineering for EA FC 26 Player Price Prediction.

Converts raw player data into numbers for ML models.
...
# Check if features have already been printed in main.py
...
def load_data(file_path: Optional[Path] = None) -> pd.DataFrame:
    ...
    if not _FeatureConfig.logged:  # So there is no empty lines in the main.py terminal
        print()
    return df
...
def create_nationality_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    nationality_dummies = pd.get_dummies(
        df["nationality_grouped"], prefix="nat", dtype=int
    )

    # Rename columns to lowercase with underscores
    nationality_dummies.columns = [
        col.lower().replace(" ", "_") for col in nationality_dummies.columns
    ]

    # Ensure all expected columns exist (safety for train/test consistency)
    expected_cols = [
        f'nat_{c.lower().replace(" ", "_")}' for c in TOP_NATIONALITIES + ["OTHER"]
    ]
    ...
    nationality_dummies = nationality_dummies.reindex(
        sorted(nationality_dummies.columns), axis=1
    )

    logger.info(
        "  %-12s: %3d features (Top 10 + 'OTHER' - representing 70%% of players)",
        "Nationality",
...
def create_league_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(
        "  %-12s: %3d features (Top 8 + 'OTHER' - representing 75%% of players)",
        "Leagues",
        len(league_dummies.columns),
    )
...
def create_card_category_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    category_dummies.columns = [
        col.lower().replace(" ", "_") for col in category_dummies.columns
    ]
    ...
    # Sort columns for consistency
    category_dummies = category_dummies.reindex(
        sorted(category_dummies.columns), axis=1
    )

    logger.info(
        "  %-12s: %3d features (Gold, Special, Icons_Heroes)",
        "Category",
        len(category_dummies.columns),
    )
    return category_dummies


def create_club_encoding_map(
    df: pd.DataFrame, target_col: str, smoothing: int = DEFAULT_SMOOTHING
) -> Dict[str, float]:
    """
    ...
    logger.info(
        "  %-12s: %3d encoded (Target Encoding, smooth=10)",
        "Clubs",
        len(club_encoding_map) - 1,
    )
    if not _FeatureConfig.logged:  # So there is no empty lines in the main.py terminal
        print()
...
def apply_club_encoding(
    df: pd.DataFrame, club_encoding_map: Dict[str, float]
) -> pd.Series:
    """
...
def prepare_features(
...
) -> Tuple[pd.DataFrame, pd.Series, StandardScaler, Dict[str, float], List[str]]:
    """
    ...
    logger.info(
        "  %-12s: %3d features (Standardized)", "Numerical", len(NUMERICAL_FEATURES)
    )

    # Position clusters (binary, as-is)
    x_positions = df[POSITION_CLUSTERS].copy()
    logger.info(
        "  %-12s: %3d clusters (Center Backs, Fullbacks, Midfielders, "
        "Attacking Midfielders, Strikers)",
        "Position",
        len(POSITION_CLUSTERS),
    )
    ...
    x = pd.concat(
        [x_numerical, x_positions, x_nationality, x_league, x_card, x_club], axis=1
    )
    ...
    # So there is no empty lines in the main.py terminal
    if logger.isEnabledFor(logging.INFO):
        print()
    logger.info(
        "Feature preparation complete: %d players with %d features",
        len(x),
        len(feature_names),
    )
    if logger.isEnabledFor(logging.INFO):
