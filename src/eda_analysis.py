...
if __name__ == "__main__":
    try:
        ...
        logger.info(f"Dataset loaded: {df.shape[0]} players, {df.shape[1]} columns")
        
        # basic price statistics
        logger.info("Price statistics (for week 1 prices):")
        ...
        logger.info("Generating price distribution visualization")
        ...
        logger.info("Saved to results/figures/01_price_distribution.png")
        plt.close()
        
        
        #RATING VS PRICE
        logger.info("Generating rating vs price visualization")
        ...
        logger.info("Saved to results/figures/02_rating_vs_price.png")
        plt.close()
        
        # correlation between rating and price
        corr_rating = df[['rating', 'price_w1']].corr().iloc[0, 1]
        logger.info(f"Correlation between rating and price: {corr_rating:.3f}")
        
        
        #PRICE BY CARD CATEGORY
        logger.info("Generating price by card category visualization")
        ...
        logger.info("Saved to results/figures/03_price_by_category.png")
        plt.close()
        
        # average price by category
        logger.info("Average price by card category:")
        category_stats = df.groupby('card_category')['price_w1'].agg(['mean', 'median', 'count'])
        print(category_stats)
        
        
        #PRICE BY POSITION CLUSTER
        logger.info("Generating price by position cluster visualization")
        ...
        logger.info("Saved to results/figures/04_price_by_position.png")
        plt.close()
        ...
        logger.info("Correlation with price:")
        print(price_corr)
        ...
        logger.info("Saved to results/figures/05_correlation_matrix.png")
        plt.close()
        
        
        #TOP 4 ATTRIBUTES COMBINED
        logger.info("Generating top 4 attributes combined visualization")
        ...
        logger.info("Saved to results/figures/06_top4_attributes.png")
        plt.close()
        
        
        #SUMMARY
        logger.info(f"Generated 6 visualizations in results/figures/")
        logger.info(f"Dataset: {len(df)} players")
        logger.info(f"Price range: {df['price_w1'].min():,.0f} - {df['price_w1'].max():,.0f}")
        logger.info(f"Strongest correlation with price: {price_corr.index[1]} ({price_corr.iloc[1]:.3f})")
        ...
        logger.error(f"Column not found: {e}") # in case a column is missing
        logger.error("Please check that all required columns exist in the dataset")
