"""
Model 1: Linear Regression on log-transformed prices.

Trains on Week 1 prices, tests on Week 2 prices.
Uses feature_engineering.prepare_features() to avoid data leakage.

Generates:
* Table: Linear Regression predictions on Week 2 (CSV)

- this module focuses on prediction (train W1/test W2, MAE, RMSE, R²)
- ols_coefficient_analysis.py focuses on interpretation (p-values, coefficients, inference)
"""
...
def main() -> None:
    ...
    try:
        ...
        logger.info("Hyperparameters: default (OLS, no regularization)")
        print()
        ...
        results_path = (
            ...
            / "results"
            / "predictions"
            / "predictions_linear_regression_w2.csv"
        )
        ...
        # Display results
        logger.info("Linear Regression Performance:")
        logger.info("-" * 80)
        ...
        logger.info("-" * 80)
        logger.info(
            "  → Saved: results/predictions/predictions_linear_regression_w2.csv"
        )
        ...
        logger.error("Unexpected error during linear regression: %s", e)
        raise
